...
describe("Aksara", () => {
  ...
  test("uppercase letters select Murda (prestige) consonant forms", () => {
    // 'HANACARAKA' — H (none), A (vowel), N (Na Murda), A (vowel),
    // C (Ca Murda), A (vowel), R (Ra Agung), A (vowel), K (Ka Murda), A (vowel)
    const upper = new Aksara("HANACARAKA").getAksara();
    const lower = new Aksara("hanacaraka").getAksara();
    expect(upper).not.toBe(lower);
    expect(upper).toBe("ꦲꦟꦖꦬꦑ");
  });

  test("mixed-case: first-letter capital uses Murda where available", () => {
    // 'Simon' — S has a Murda form (Sa Murda)
    const aksara = new Aksara("Simon");
    expect(aksara.getAksara()).toBe("ꦯꦶꦩꦺꦴꦤ꧀");
  });

  test("Ka Murda: uppercase K", () => {
    expect(new Aksara("Ka").getAksara()).toBe("ꦑ");
  });

  test("lowercase k uses regular Ka", () => {
    expect(new Aksara("ka").getAksara()).toBe("ꦏ");
  });

  test("Na Murda: uppercase N", () => {
    expect(new Aksara("Na").getAksara()).toBe("ꦟ");
  });

  test("Sa Murda: uppercase S", () => {
    expect(new Aksara("Sa").getAksara()).toBe("ꦯ");
  });

  test("Pa Murda: uppercase P", () => {
    expect(new Aksara("Pa").getAksara()).toBe("ꦦ");
  });

  test("Ra Agung: uppercase R", () => {
    expect(new Aksara("Ra").getAksara()).toBe("ꦬ");
  });

  test("Ga Murda: uppercase G", () => {
    expect(new Aksara("Ga").getAksara()).toBe("ꦓ");
  });

  test("consonant without Murda form falls back to regular when uppercase", () => {
    // 'W' has no Murda form
    expect(new Aksara("Wa").getAksara()).toBe("ꦮ");
    expect(new Aksara("La").getAksara()).toBe("ꦭ");
  });

  test("uppercase vowel has no Murda effect", () => {
    expect(new Aksara("A").getAksara()).toBe("ꦲ");
    expect(new Aksara("I").getAksara()).toBe("ꦲꦶ");
  });

  test("Ba Murda: uppercase B", () => {
    expect(new Aksara("Ba").getAksara()).toBe("ꦨ");
  });

  test("Ca Murda: uppercase C", () => {
    expect(new Aksara("Ca").getAksara()).toBe("ꦖ");
  });

  test("Ta Murda: uppercase T", () => {
    expect(new Aksara("Ta").getAksara()).toBe("ꦡ");
  });

  test("lowercase t uses regular Ta", () => {
    expect(new Aksara("ta").getAksara()).toBe("ꦠ");
  });

  test("Nya Murda: uppercase Ny", () => {
    expect(new Aksara("Nya").getAksara()).toBe("ꦘ");
  });

  test("lowercase nya uses regular Nya", () => {
    expect(new Aksara("nya").getAksara()).toBe("ꦚ");
  });

  test("Murda at start of proper name: Surabaya", () => {
    expect(new Aksara("Surabaya").getAksara()).toBe("ꦯꦸꦫꦧꦪ");
  });

  test("Murda combined with vowel diacritics", () => {
    // Ki with regular K, Ki with Murda K
    expect(new Aksara("Ki").getAksara()).toBe("ꦑꦶ");
    expect(new Aksara("Ku").getAksara()).toBe("ꦑꦸ");
  });

  test("Murda in wyanjana chain uses regular pasangan (not Murda)", () => {
    // 'Ska' — S is Murda, k in wyanjana uses regular pasangan
    expect(new Aksara("Ska").getAksara()).toBe("ꦯ꧀ꦏ");
  });

  test("lowercase-only text is unaffected", () => {
    expect(new Aksara("hanacaraka").getAksara()).toBe("ꦲꦤꦕꦫꦏ");
    expect(new Aksara("aji saka", true).getAksara()).toBe("ꦲꦗꦶ ꦱꦏ");
  });
...
describe("Aksara.fromAksara", () => {
  ...
  test("roundtrip: hanacaraka", () => {
    expect(Aksara.fromAksara(new Aksara("hanacaraka").getAksara())).toBe(
      "hanacaraka",
    );
  });

  test("roundtrip: wong jawa with spaces", () => {
    expect(Aksara.fromAksara(new Aksara("wong jawa", true).getAksara())).toBe(
      "wong jawa",
    );
  });
