...
class OcrDataset(Dataset):
    """
    Simple PyTorch Dataset that pairs PNG/JPG images with their corresponding
    ...
    def __init__(self, data_dir: Path, augment: bool = False):
        ...
        # Collect all image files; ensure a corresponding .txt exists
        self.samples: List[Tuple[Path, Path]] = []
        exts = ("*.png", "*.PNG", "*.jpg", "*.JPG", "*.jpeg", "*.JPEG")
        img_files = sorted({p.resolve() for ext in exts for p in self.data_dir.glob(ext)})
